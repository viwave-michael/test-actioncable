...
class CommandsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create command" do
    assert_difference('Command.count') do
      post commands_url, params: { command: { action: @command.action, device_id: @command.device_id } }
    end
  ...
  test "should update command" do
    patch command_url(@command), params: { command: { action: @command.action, device_id: @command.device_id } }
